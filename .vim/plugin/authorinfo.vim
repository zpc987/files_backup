...
"  Description:     
"  Version:         1.5
"  LastChange:      2013-01-26 15:55:15-changed by zpc ,more easy to use
"     History:      2011-02-23 16:42:42
...
function s:AddTitle()
    ...
    let gotoLn = line('.')
    normal o
    call setline('.',noTypeChar.preChar.'    CreatTime: '.strftime("%Y-%m-%d %H:%M:%S"))
    normal o
    call setline('.',noTypeChar.'=============================================================================')
    let lastLine = line('.')
...
function s:TitleDet()
    ...
    while (n < 25)
        ...
        if line =~ '^.*LastChange:\S*.*$'
            ...
            call setline(n,newline)
			let gotoAddLn = line('.')
            let oldline=substitute(line,'LastChange','   History','g')
            exe 'normal '.n.'G'
            normal yy p
			let m = n+1
            call setline(m,oldline)
            let updated = 1
        endif
        let n = n + 1
    endwhile
